...
const holdem_pc = require("./Pacific");

function getLines(txt: string) {
  const trimmed: string[] = txt.split("\n").map(stringUtil.trimLine);
...
function parseHand(input: string) {
  ...
  if (holdem_pc.canParse(lines)) return holdem_pc.parse(lines);
}
 ...
 */
function canParse(input: string) {
  const lines = nonEmptyLines(input);
  if (lines == null || lines.length === 0) return false;
  return /*holdem_ps.canParse(lines) || holdem_ig.canParse(lines) || holdem_pp.canParse(lines) ||*/ holdem_pc.canParse(
    lines
  );
}
 ...
 */
function extractHands(txt: string): string[][] {
...
//  */
// function parseHands(txt: string) {
...
//   //   try {
//   //     const parsedHand = parseHand(hands[i]);
//   //     if (parsedHand == null) {
...
export { parseHand, extractHands, canParse };
